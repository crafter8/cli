...
export async function removeCliProfile(apiBaseUrl = null, env = process.env) {
  const config = await loadCliConfig(env);
  const selectedProfile = selectCliProfile(config, apiBaseUrl);
  if (!selectedProfile) {
    return {
      removed: false,
  ...
  delete config.profiles[selectedProfile.apiBaseUrl];
  const remainingKeys = Object.keys(config.profiles);
  config.defaultProfile =
    config.defaultProfile === selectedProfile.apiBaseUrl
      ? remainingKeys[0] || null
      : normalizeBaseUrl(config.defaultProfile);
  const configPath = await saveCliConfig(config, env);
  return {
    removed: true,
    configPath,
...
export async function clearCliProfiles(env = process.env) {
  const config = createEmptyCliConfig();
  const configPath = await saveCliConfig(config, env);
  return {
    removed: true,
